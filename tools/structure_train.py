...
from core.loss import StructureLoss
...
def main():
    ...
    criterion = StructureLoss(
        use_target_weight=cfg.LOSS.USE_TARGET_WEIGHT, dataset=cfg.DATASET.DATASET
    ).cuda()
    ...
    optimizer = get_optimizer(cfg, model, joints_weight)
    ...
    joints_weight_epoch = [i for i in range(100, cfg.TRAIN.END_EPOCH, 20)]
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        # train for one epoch
        wandb_train_log = train(cfg, train_loader, model, criterion, optimizer, epoch, joints_weight,
                                final_output_dir, tb_log_dir, wandb, writer_dict)

        # evaluate on validation set
        perf_indicator, wandb_val_log = validate(
            cfg, valid_loader, valid_dataset, model, criterion,
            final_output_dir, joints_weight, tb_log_dir, wandb, writer_dict
        )
        ...
        lr_scheduler.step()
        optimizer.param_groups[2]['lr'] = cfg.TRAIN.JOINTS_LR
