...
from utils.data_container import DataContainer as DC

logger = logging.getLogger(__name__)


class JointsDatasetMOE(Dataset):
    def __init__(self, cfg, root, image_set, is_train, transform=None, data_cfg=None):
        ...
        self.rotation_factor = data_cfg['rot_factor']
        ...
        self.max_num_joints = data_cfg['max_num_joints'] if 'max_num_joints' in data_cfg else None
        self.dataset_idx = data_cfg['dataset_idx']
    ...
    def generate_target(self, joints, joints_vis):
        ...
        if self.max_num_joints is not None:
            padded_length = int(self.max_num_joints - self.num_joints)
            target_weight = np.concatenate([target_weight, np.zeros((padded_length, 1), dtype=np.float32)], 0)
            target = np.concatenate([target, np.zeros((padded_length, self.heatmap_size[1], self.heatmap_size[0]), dtype=np.float32)], 0)
