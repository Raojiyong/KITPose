...
from dataset.JointsDatasetMOE import JointsDatasetMOE
...
class AnimalposeDatasetMOE(JointsDatasetMOE):
    def __init__(self, cfg, root, image_set, is_train, transform=None, data_cfg=None):
        super().__init__(cfg, root, image_set, is_train, transform, data_cfg)
    ...
    def _get_ann_file_keypoint(self):
        """ self.root / annotations / animalpose_train.json """
        prefix = 'animalpose' \
    ...
    def _load_coco_keypoint_annotations(self):
        ...
        for index in self.image_set_index:
            db_tmp = self._load_coco_keypoint_annotation_kernal(index)
            gt_db.extend(db_tmp)
        return gt_db
    ...
    def _load_coco_keypoint_annotation_kernal(self, index):
        ...
        for obj in objs:
            ...
            image_file = os.path.join(self.root, self.id2name[index])
            rec.append({
                'image': image_file,
                'center': center,
                'scale': scale,
    ...
    def evaluate(self, cfg, preds, output_dir, all_boxes, bbox_ids, img_path,
        ...
        for idx, kpt in enumerate(preds):
            # im_id = img_path[idx][len(self.root):]
            image_id = self.name2id[img_path[idx][len(self.root):]]
            ...
            result = [
                {
                    ...
                    # 'image_path': img_kpts[k]['image_path'],
                    'category_id': cat_id,
