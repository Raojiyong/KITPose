...
def build_scheduler(config, optimizer, n_iter_per_epoch):
    ...
    multi_steps = [i * n_iter_per_epoch for i in config.TRAIN.LR_STEP]

    lr_scheduler = None
    if config.TRAIN.LR_SCHEDULER == 'CosineLR':
        lr_scheduler = CosineLRScheduler(
            optimizer,
            t_initial=num_steps,
            lr_min=config.TRAIN.LR_END,
            warmup_lr_init=config.TRAIN.WARMUP_LR,
            warmup_t=warmup_steps,
            cycle_limit=1,
            t_in_epochs=False,
        )
    elif config.TRAIN.LR_SCHEDULER == 'LinearLR':
    ...
    #         t_in_epochs=False,
    #     )
    elif config.TRAIN.LR_SCHEDULER == 'MultiStepLR':
