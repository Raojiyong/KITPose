...
class DWConv(nn.Module):
    def __init__(self, dim=17, patch_size=[64, 64]):
    ...
    def forward(self, x):
        ...
        x = x.view(B, N, H, W)  # rearrange(x, ' b n c -> b n h w ', h=HW, w=HW)
        # HW = int(math.sqrt(C))
        # x = x.view(B, N, HW, HW)
        x = self.dwconv(x)
        x = x.flatten(2)
...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, in_channels=17, act_layer=nn.GELU, dropout=0., patch_size=[64, 64]):
        super().__init__()
        self.fc1 = nn.Sequential(nn.Linear(dim, hidden_dim),
                                 act_layer())
        self.drop = nn.Dropout(dropout)
        self.act = act_layer()
        self.dwconv = nn.Sequential(DWConv(in_channels, patch_size=patch_size),
                                    act_layer())
...
class PoseEncoderLayer(nn.Module):
    def __init__(self, dim, heads, dropout, mlp_dim, num_keypoints=None, all_attn=False, scale_with_head=False,
        ...
        self.ffn = FeedForward(dim, mlp_dim, in_channels=num_keypoints, dropout=dropout, patch_size=patch_size)
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, mlp_dim, dropout, num_keypoints=None, all_attn=False, scale_with_head=False,
        ...
        self.layers = nn.ModuleList([
            PoseEncoderLayer(dim, heads, dropout, mlp_dim, num_keypoints, all_attn, scale_with_head, patch_size)
...
class KITPose_base(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False,
        ...
        self.transformer = Transformer(dim, depth, heads, mlp_dim, dropout, num_keypoints=num_keypoints,
                                       all_attn=self.all_attn, scale_with_head=False, patch_size=patch_size)
