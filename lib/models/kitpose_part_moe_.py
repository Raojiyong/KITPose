...
from .pose_hrnet import PoseHighResolutionNet2, Bottleneck, BasicBlock
...
class KITPose_base(nn.Module):
    def __init__(self, *, feature_size, kpt_size, num_keypoints, num_bp, dim, inner_dim, depth, heads, mlp_dim,
                 ...
                 aia_mode=False, final_conv_kernel=1, max_num_kpt=17):
        super().__init__()
        ...
        # transformer
        self.transformer = Transformer(dim, inner_dim, depth, heads, mlp_dim, dropout, num_keypoints=self.num_keypoints,
                                       num_bp=self.num_bp, all_attn=self.all_attn, scale_with_head=False, aia_mode=aia_mode)

        self.to_keypoint_token = nn.Identity()

        self.kpt_head = nn.Conv2d(
        ...
        self.prompt_learner = PromptLearner(self.num_bp, heatmap_dim, dim, self.inplanes)
    ...
    def forward(self, all_feature, kpt_feature=None, mask=None, indices=None):
        p = self.patch_size
        # transformer
        kpt_out = self.kpt_head(all_feature)
        # kpt_out = kpt_out[:, :self.num_keypoints]
        kpt_feature = rearrange(kpt_out, 'b c p1 p2 -> b c (p1 p2)', p1=p[0], p2=p[1])
        ...
        x = self.transformer(x, pos=self.pos_embedding)
        x = self.mlp_head(x)
        ...
        x = torch.cat([x, torch.zeros((b, self.max_num_kpt - n, self.heatmap_size[0], self.heatmap_size[1]), dtype=x.dtype, device=x.device)], dim=1)
        kpt_out = torch.cat([
            kpt_out, torch.zeros((b, self.max_num_kpt - n, self.heatmap_size[0], self.heatmap_size[1]), dtype=kpt_out.dtype, device=kpt_out.device)],
            dim=1)
        return kpt_out, bp_embeddings, x
...
class KITPose(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        for i in range(self.num_experts):
            channel_formers.append(KITPose_base(
                ...
                max_num_kpt=cfg.MODEL.MAX_NUM_JOINTS,
            ))
        self.channelformer = nn.ModuleList(channel_formers)
        # self.channelformer = KITPose_base(
        #     feature_size=[cfg.MODEL.IMAGE_SIZE[1] // 4, cfg.MODEL.IMAGE_SIZE[0] // 4],
        #     kpt_size=[cfg.MODEL.PATCH_SIZE[1], cfg.MODEL.PATCH_SIZE[0]],
        #     num_keypoints=cfg.MODEL.NUM_JOINTS[0], num_bp=cfg.MODEL.NUM_BP[0],
        #     dim=cfg.MODEL.DIM, inner_dim=cfg.MODEL.INNER_DIM,
        #     channels=extra.STAGE2.NUM_CHANNELS[0],
        #     depth=cfg.MODEL.TRANSFORMER_DEPTH, heads=cfg.MODEL.TRANSFORMER_HEADS,
        #     mlp_dim=cfg.MODEL.DIM * cfg.MODEL.TRANSFORMER_MLP_RATIO,
    ...
    def forward(self, x, indices=None):
        ...
        # shared_feats = self.pre_feature(x)
        kpt_feats, outs = [], []
        for i in range(self.num_experts):
            kpt_feat, _, out = self.channelformer[i](x)
            kpt_feats.append(kpt_feat)
            outs.append(out)
        return kpt_feats, outs
