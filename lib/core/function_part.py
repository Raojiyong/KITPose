...
def train(config, train_loader, model, criterion_kpts, criterion_parts, optimizer, epoch,
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        ...
        # compute output
        im_kpt_feats, im_feats, outputs = model(input)
        ...
        if isinstance(outputs, list):
            kpt_loss = criterion_kpts(outputs[0], target, target_weight)
            for output in outputs[1:]:
                kpt_loss += criterion_kpts(output, target, target_weight)
        else:
            output = outputs
            kpt_loss = criterion_kpts(output, target, target_weight)
        ...
        # body part loss
        part_loss = criterion_parts(im_kpt_feats, target, target_weight)
