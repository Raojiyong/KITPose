...
# ---------------------------------------------------------------------------------
def cutout(input, target, target_weight, alpha=1.0, cutout_prob=1.0):
    r = np.random.rand(1)
    if alpha > 0 and r < cutout_prob:
        lam = np.random.beta(alpha, alpha)
    else:
        lam = -1
        return input, target, target_weight, None

    bbx1, bby1, bbx2, bby2 = rand_bbox(input.size(), lam)
    bbx1_t, bby1_t, bbx2_t, bby2_t = bbx1 // 4, bby1 // 4, bbx2 // 4, bby2 // 4

    input_cutout = input.clone()
    target_cutout = target.clone()
    target_weight_cutout = target_weight.clone().view(-1, target.size(1))

    input_cutout[:, :, bbx1:bbx2, bby1:bby2] = 0.0
    target_cutout[:, :, bbx1_t:bbx2_t, bby1_t:bby2_t] = 0.0

    # update to mask keypoint inside box
    keypoints, max_vals = get_max_preds(target.clone().cpu().numpy())
    ...
    kps_outside_bbox = torch.tensor((~kps_inside_bbox) * 1.0)
    target_weight_cutout = (kps_outside_bbox * target_weight_cutout).view(-1, target.size(1), 1)

    return input_cutout, target_cutout, target_weight_cutout
...
def train_cutout(config, train_loader, model, criterion_kpts, criterion_parts, optimizer, epoch,
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        ...
        co_input, co_target, co_twei = \
            cutout(input, target, target_weight, alpha=1.0, cutout_prob=1.0)

        # compute output
        im_kpt_feats, im_feats, outputs = model(co_input)

        co_target = co_target.cuda(non_blocking=True)
        co_twei = co_twei.cuda(non_blocking=True)

        # keypoints loss
        if isinstance(outputs, list):
            kpt_loss = criterion_kpts(outputs[0], co_target, co_twei)
            for output in outputs[1:]:
                kpt_loss += criterion_kpts(output, co_target, co_twei)
        else:
            output = outputs
            kpt_loss = criterion_kpts(output, co_target, co_twei)
        ...
        # body part loss
        part_loss = criterion_parts(im_kpt_feats, co_target, co_twei)
        ...
        if i % config.PRINT_FREQ == 0:
            ...
            save_debug_images(config, co_input, meta, co_target, pred * 4, output,
                              prefix)
    wandb_train_log = {
