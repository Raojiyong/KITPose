...
def validate(config, val_loader, val_dataset, model, criterion, output_dir, wandb=None, log_type='map'):
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_weight, meta) in enumerate(val_loader):
            # compute output
            _, bps, outputs, c_time = model(input)
            ...
            if config.TEST.FLIP_TEST:
                ...
                _, _, outputs_flipped, _ = model(input_flipped)
            ...
            # measure elapsed time
            batch_time.update(time.time() - end)
            cluster_time.update(c_time, num_images)
            end = time.time()
            ...
            if i % config.PRINT_FREQ == 0:
                msg = 'Test: [{0}/{1}]\t' \
                      ...
                      'Accuracy {acc.val:.3f} ({acc.avg:.3f})'.format(
                    i, len(val_loader), batch_time=batch_time, cluster_time=cluster_time,
                    loss=losses, acc=acc)
                logger.info(msg)
            ...
            if i >= num_warmup:
                pure_inf_time += c_time
                its = (i + 1 - num_warmup) / pure_inf_time
        logger.info(f'Clustering average: {its:.2f} items / s')
        logger.info(f'Total time: {pure_inf_time:.2f} s')
